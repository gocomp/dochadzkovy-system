...
begin
  ...
  begin
    // Get alarm
    extraData := gDeviceTools.GetBytesByNum(IntToStr(DeviceAlarmClock),
      NumberType_Int32Bit);
    bRet := gDeviceConnection.GetPropertyExt_2(DeviceProperty_Bell,
...
begin
  ...
  begin
    ...
    begin
      // Set alarm
      extraProperty := gDeviceTools.GetBytesByNum(IntToStr(DeviceAlarmClock),
        NumberType_Int32Bit);
      iIndex := cbbAlarmDN.ItemIndex * BellLength;
      SetArrayValue(extraData, iIndex + 0,
        StrToInt(FormatDateTime('hh', dtpAlarmTime.Time)));
      SetArrayValue(extraData, iIndex + 1,
        StrToInt(FormatDateTime('nn', dtpAlarmTime.Time)));
      SetArrayValue(extraData, iIndex + 2, cbbAlarmCycle.ItemIndex);
      SetArrayValue(extraData, iIndex + 3, StrToInt(edtAlarmDelay.Text));
      bRet := gDeviceConnection.SetPropertyExt_2(DeviceProperty_Bell,
        extraProperty, gIDevice, extraData);
      if bRet then
