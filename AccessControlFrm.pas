...
begin
  ...
  begin
    ...
    iType := 72;
    extraData := gDeviceTools.GetBytesByNum(IntToStr(iType),
      NumberType_Int32Bit);
    bRet := gDeviceConnection.GetPropertyExt_2
      (DeviceProperty_AccessControlSettings, extraProperty, gIDevice,
      extraData);
    if bRet then
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      extraProperty := gDeviceTools.GetBytesByNum(IntToStr(iType),
        NumberType_Int32Bit);
      ...
      bRet := gDeviceConnection.SetPropertyExt_2
        (DeviceProperty_AccessControlSettings, extraProperty, gIDevice,
        extraData);
      if bRet then
...
begin
  ...
  begin
    ...
    extraData := gDeviceTools.GetBytesByNum(IntToStr(iType),
      NumberType_Int32Bit);
    bRet := gDeviceConnection.GetPropertyExt_2(DeviceProperty_PassSegment,
      extraProperty, gIDevice, extraData);
    if bRet then
    begin
      for i := 0 to 31 do
      begin
        ldt := (scrlbxSegment.FindComponent('edtStTime' + IntToStr(i))
          as TDateTimePicker);
        GetArrayValue(extraData, i * 4, bData0);
        GetArrayValue(extraData, i * 4 + 1, bData1);
        ldt.Time := EncodeTime(bData0, bData1, 0, 0);
        ldt := (scrlbxSegment.FindComponent('edtEndTime' + IntToStr(i))
          as TDateTimePicker);
  ...
  begin
    ...
    begin
      ...
      begin
        ldt := (scrlbxSegment.FindComponent('edtStTime' + IntToStr(i))
          as TDateTimePicker);
        SetArrayValue(extraData, i * 4,
          StrToInt(FormatDateTime('hh', ldt.Time)));
        SetArrayValue(extraData, i * 4 + 1,
          StrToInt(FormatDateTime('nn', ldt.Time)));
        ldt := (scrlbxSegment.FindComponent('edtEndTime' + IntToStr(i))
          as TDateTimePicker);
        SetArrayValue(extraData, i * 4 + 2,
          StrToInt(FormatDateTime('hh', ldt.Time)));
        SetArrayValue(extraData, i * 4 + 3,
          StrToInt(FormatDateTime('nn', ldt.Time)));
      end;
      bRet := gDeviceConnection.SetPropertyExt_2(DeviceProperty_PassSegment,
        extraProperty, gIDevice, extraData);
      if bRet then
...
begin
  ...
  begin
    ...
    extraData := gDeviceTools.GetBytesByNum(IntToStr(iType),
      NumberType_Int32Bit);
    bRet := gDeviceConnection.GetPropertyExt_2(DeviceProperty_PassGroup,
      extraProperty, gIDevice, extraData);
    if bRet then
    begin
      for i := 0 to 31 do
      begin
        for j := 0 to 9 do
        begin
          lEdit := (scrlbxGroup.FindComponent('edtGroup' + IntToStr(i) +
            IntToStr(j)) as TEdit);
...
begin
  ...
  begin
    ...
    begin
      ...
      bRet := gDeviceConnection.SetPropertyExt_2(DeviceProperty_PassGroup,
        extraProperty, gIDevice, extraData);
      if bRet then
...
begin
  ...
  begin
    ...
    extraData := gDeviceTools.GetBytesByNum(IntToStr(iType),
      NumberType_Int32Bit);
    bRet := gDeviceConnection.GetPropertyExt_2(DeviceProperty_PassZone,
      extraProperty, gIDevice, extraData);
    if bRet then
    begin
      for i := 0 to 31 do
      begin
        for j := 0 to 6 do
        begin
          lEdit := (scrlbxZone.FindComponent('edtZone' + IntToStr(i) +
            IntToStr(j)) as TEdit);
...
begin
  ...
  begin
    ...
    begin
      ...
      bRet := gDeviceConnection.SetPropertyExt_2(DeviceProperty_PassZone,
        extraProperty, gIDevice, extraData);
      if bRet then
...
begin
  ...
  begin
    ...
    extraData := gDeviceTools.GetBytesByNum(IntToStr(iType),
      NumberType_Int32Bit);
    bRet := gDeviceConnection.GetPropertyExt_2(DeviceProperty_MultOpenGroup,
      extraProperty, gIDevice, extraData);
    if bRet then
    begin
      for i := 0 to 9 do
      begin
        lEdit := (scrlbxMultOpenGroup.FindComponent('edtMultOpen' + IntToStr(i))
          as TEdit);
        GetArrayValue(extraData, i * 4, bData0);
        GetArrayValue(extraData, i * 4 + 1, bData1);
        GetArrayValue(extraData, i * 4 + 2, bData2);
        GetArrayValue(extraData, i * 4 + 3, bData3);
        lEdit.Text := IntToStr(bData3 * $1000000 + bData2 * $10000 + bData1 *
          $100 + bData0);
      end;
    end
    else
...
begin
  ...
  begin
    ...
    begin
      ...
      bRet := gDeviceConnection.SetPropertyExt_2(DeviceProperty_MultOpenGroup,
        extraProperty, gIDevice, extraData);
      if bRet then
...
begin
  ...
  if cbbLockCombo.ItemIndex > 0 then
    iIndex := 42;
  deviceStatus := gDeviceTools.GetBytesByNum(IntToStr(iIndex),
    NumberType_Int32Bit);
  ArrayCopy(deviceStatus, 0, extraData, 0, 4);
  ArrayCopy(gDeviceTools.GetBytesByNum(IntToStr(3), NumberType_Int32Bit), 0,
    extraData, 4, 4);
...
begin
  ...
  deviceStatus := gDeviceTools.GetBytesByNum(IntToStr(iIndex),
    NumberType_Int32Bit);
  ArrayCopy(deviceStatus, 0, extraData, 0, 4);
  ArrayCopy(gDeviceTools.GetBytesByNum(IntToStr(1), NumberType_Int32Bit), 0,
    extraData, 4, 4);
  bRet := gDeviceConnection.SetPropertyExt_2(DeviceProperty_SysParam,
    extraProperty, gIDevice, extraData);
  if bRet then
...
begin
  ...
  deviceStatus := gDeviceTools.GetBytesByNum(IntToStr(iIndex),
    NumberType_Int32Bit);
  ArrayCopy(deviceStatus, 0, extraData, 0, 4);
  ArrayCopy(gDeviceTools.GetBytesByNum(IntToStr(2), NumberType_Int32Bit), 0,
    extraData, 4, 4);
  bRet := gDeviceConnection.SetPropertyExt_2(DeviceProperty_SysParam,
    extraProperty, gIDevice, extraData);
  if bRet then
...
begin
  ...
  deviceStatus := gDeviceTools.GetBytesByNum(IntToStr(iIndex),
    NumberType_Int32Bit);
  ArrayCopy(deviceStatus, 0, extraData, 0, 4);
  ArrayCopy(gDeviceTools.GetBytesByNum(IntToStr(4), NumberType_Int32Bit), 0,
    extraData, 4, 4);
  bRet := gDeviceConnection.SetPropertyExt_2(DeviceProperty_SysParam,
    extraProperty, gIDevice, extraData);
  if bRet then
...
begin
  ...
  ArrayCopy(deviceStatus, 0, extraData, 0, 4);
  ArrayCopy(gDeviceTools.GetBytesByNum(IntToStr(cbbSoundList.ItemIndex),
    NumberType_Int32Bit), 0, extraData, 4, 4);
  bRet := gDeviceConnection.SetPropertyExt_2(DeviceProperty_PowerOff,
    extraProperty, gIDevice, extraData);
  if bRet then
  begin
...
begin
  ...
  begin
    ...
    ShowMessage('Door sensor =' + IntToStr(bData3 * $1000000 + bData2 * $10000 +
      bData1 * $100 + bData0));
  end
  else
  begin
...
begin
  ...
  begin
    ...
    GetArrayValue(deviceStatus, 2, bData2);
    GetArrayValue(deviceStatus, 3, bData3);
    ShowMessage('Event =' + IntToStr(bData3 * $1000000 + bData2 * $10000 +
      bData1 * $100 + bData0));
  end
  else
  begin
