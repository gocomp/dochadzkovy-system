...
begin
  ...
  begin
    // Get message
    extraData := gDeviceTools.GetBytesByNum(IntToStr(DeviceMessage),
      NumberType_Int32Bit);
    bRet := gDeviceConnection.GetPropertyExt_2(DeviceProperty_Message,
    ...
    begin
      ...
      ArrayCopy(messageBytes, 24, contentBytes, 0,
        contentBytes.rgsabound[0].cElements);
      ...
      dtpBeginDate.Date := EncodeDate(bBeginYear + 2000, bBeginMonth,
        bBeginDay);
      dtpEndTime.Time := EncodeTime(bEndHour, bEndMinute, 0, 0);
      dtpEndDate.Date := EncodeDate(bEndYear + 2000, bEndMonth, bEndDay);
      edtMessageID.Text := gDeviceTools.GetStringByNum(idBytes, 0,
        NumberType_UInt64Bit);
      edtContent.Text := StringReplace(gDeviceTools.GetString(contentBytes), #0,
        '', [rfReplaceAll]);
    end
    else
    begin
...
begin
  ...
  begin
    ...
    begin
      // Set message
      extraProperty := gDeviceTools.GetBytesByNum(IntToStr(DeviceMessage),
        NumberType_Int32Bit);
      iIndex := cbbDN.ItemIndex * MaxDeviceMessageLength;
      ...
      SetArrayValue(extraData, iIndex + 6,
        StrToInt(FormatDateTime('yyyy', dtpBeginDate.Date)) - 2000);
      SetArrayValue(extraData, iIndex + 7,
        StrToInt(FormatDateTime('mm', dtpBeginDate.Date)));
      SetArrayValue(extraData, iIndex + 8,
        StrToInt(FormatDateTime('dd', dtpBeginDate.Date)));
      SetArrayValue(extraData, iIndex + 9,
        StrToInt(FormatDateTime('hh', dtpBeginTime.Time)));
      SetArrayValue(extraData, iIndex + 10,
        StrToInt(FormatDateTime('nn', dtpBeginTime.Time)));
      SetArrayValue(extraData, iIndex + 11,
        StrToInt(FormatDateTime('yyyy', dtpEndDate.Date)) - 2000);
      SetArrayValue(extraData, iIndex + 12,
        StrToInt(FormatDateTime('mm', dtpEndDate.Date)));
      SetArrayValue(extraData, iIndex + 13,
        StrToInt(FormatDateTime('dd', dtpEndDate.Date)));
      SetArrayValue(extraData, iIndex + 14,
        StrToInt(FormatDateTime('hh', dtpEndTime.Time)));
      SetArrayValue(extraData, iIndex + 15,
        StrToInt(FormatDateTime('nn', dtpEndTime.Time)));
      idBytes := gDeviceTools.GetBytesByNum(Trim(edtMessageID.Text),
        NumberType_UInt64Bit);
      ArrayCopy(idBytes, 0, extraData, cbbDN.ItemIndex * MaxDeviceMessageLength
        + 16, idBytes.rgsabound[0].cElements);
      contentBytes := gDeviceTools.GetBytes(Trim(edtContent.Text));
      ArrayCopy(contentBytes, 0, extraData,
        cbbDN.ItemIndex * MaxDeviceMessageLength + 24,
        contentBytes.rgsabound[0].cElements);
      bRet := gDeviceConnection.SetPropertyExt_2(DeviceProperty_Message,
        extraProperty, gIDevice, extraData);
      if bRet then
