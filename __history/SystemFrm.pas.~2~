...
type
  ...
  private
    ...
    function CheckWaveFormat(var Buffer: PSafeArray;
...
begin
  ...
  bRet := gDeviceConnection.SetPropertyExt_2(DeviceProperty_DeviceTime,
...
begin
  ...
  extraProperty := CreateSafeArray(2);
  deviceStatus := CreateSafeArray(2);
  bRet := gDeviceConnection.GetPropertyExt_2(DeviceProperty_DeviceTime,
    extraProperty, gIDevice, deviceStatus);
  if bRet then
  begin
    ...
    edtTime.Text := IntToStr(bYear + 2000) + '-' + IntToStr(bMonth) + '-' +
      IntToStr(bDay) + ' ' + IntToStr(bHour) + ':' + IntToStr(bMinute) + ':' +
      IntToStr(bSecond);
  end
  else
  begin
...
begin
  ...
  case cbbParam.ItemIndex of
    26 .. 29:
      begin
        paramValue := gDeviceTools.GetBytesByNum
          (gDeviceTools.ConvertNumberToIPAddress
          (StrToInt(Trim(edtParamValue.Text))), NumberType_Int32Bit);
      end;
  else
    paramValue := gDeviceTools.GetBytesByNum(Trim(edtParamValue.Text),
      NumberType_Int32Bit);
  end;
  paramIndex := gDeviceTools.GetBytesByNum(IntToStr(cbbParam.ItemIndex),
    NumberType_Int32Bit);
  ...
  ArrayCopy(paramValue, 0, extraData, paramIndex.rgsabound[0].cElements,
    paramValue.rgsabound[0].cElements);
  bRet := gDeviceConnection.SetPropertyExt_2(DeviceProperty_SysParam,
...
begin
  ...
  extraProperty := CreateSafeArray(2);
  deviceStatus := gDeviceTools.GetBytesByNum(IntToStr(cbbParam.ItemIndex),
    NumberType_Int32Bit);
  bRet := gDeviceConnection.GetPropertyExt_2(DeviceProperty_SysParam,
    extraProperty, gIDevice, deviceStatus);
  if bRet then
  begin
    paramValue := StrToInt(gDeviceTools.GetStringByNum(deviceStatus, 0,
      NumberType_Int32Bit));
    case cbbParam.ItemIndex of
      26 .. 29:
        begin
          edtParamValue.Text := gDeviceTools.ConvertNumberToIPAddress
            (paramValue);
        end;
    else
      edtParamValue.Text := IntToStr(paramValue);
    end;
  end
  else
  begin
...
begin
  ...
  begin
    // Get power time
    extraData := gDeviceTools.GetBytesByNum(IntToStr(DevicePowerTimer),
      NumberType_Int32Bit);
    bRet := gDeviceConnection.GetPropertyExt_2(DeviceProperty_PowerOnOffTime,
      extraProperty, gIDevice, extraData);
    if bRet then
    begin
      iIndex := (cbbPowerType.ItemIndex + 1) * PowerTimeCount * PowerTimeLength
        + cbbDN.ItemIndex * PowerTimeLength;
...
begin
  ...
  begin
    ...
    if bRet then
    begin
      extraProperty := gDeviceTools.GetBytesByNum(IntToStr(DevicePowerTimer),
        NumberType_Int32Bit);
      iIndex := (cbbPowerType.ItemIndex + 1) * PowerTimeCount * PowerTimeLength
        + cbbDN.ItemIndex * PowerTimeLength;
      SetArrayValue(extraData, iIndex + 0,
        StrToInt(FormatDateTime('hh', dtpPowerTime.Time)));
      SetArrayValue(extraData, iIndex + 1,
        StrToInt(FormatDateTime('nn', dtpPowerTime.Time)));
      if chkPowerTimeOff.Checked then
        bEnable := 1
      else
        bEnable := 0;
      SetArrayValue(extraData, iIndex + 2, bEnable);
      bRet := gDeviceConnection.SetPropertyExt_2(DeviceProperty_PowerOnOffTime,
        extraProperty, gIDevice, extraData);
      if bRet then
...
begin
  // System Parameters get
  extraProperty := CreateSafeArray(2);
  deviceStatus := gDeviceTools.GetBytesByNum(IntToStr(DeviceProperty_Status),
    NumberType_Int32Bit);
  bRet := gDeviceConnection.GetPropertyExt_2(DeviceProperty_Status,
    extraProperty, gIDevice, deviceStatus);
  if bRet then
  begin
    edtUserCount.Text := gDeviceTools.GetStringByNum(deviceStatus, 0,
      NumberType_UInt32Bit);
    edtAdminCount.Text := gDeviceTools.GetStringByNum(deviceStatus, 4,
      NumberType_UInt32Bit);
    edtFingerCount.Text := gDeviceTools.GetStringByNum(deviceStatus, 8,
      NumberType_UInt32Bit);
    edtCardCount.Text := gDeviceTools.GetStringByNum(deviceStatus, 12,
      NumberType_UInt32Bit);
    edtPwdCount.Text := gDeviceTools.GetStringByNum(deviceStatus, 16,
      NumberType_UInt32Bit);
    edtSLogCount.Text := gDeviceTools.GetStringByNum(deviceStatus, 20,
      NumberType_UInt32Bit);
    edtGLogCount.Text := gDeviceTools.GetStringByNum(deviceStatus, 24,
      NumberType_UInt32Bit);
    edtHisSLogCount.Text := gDeviceTools.GetStringByNum(deviceStatus, 28,
      NumberType_UInt32Bit);
    edtHisGLogCount.Text := gDeviceTools.GetStringByNum(deviceStatus, 32,
      NumberType_UInt32Bit);
  end
  else
  begin
...
begin
  ...
  begin
    // reset device
    bRet := gDeviceConnection.SetPropertyExt_2(DeviceProperty_InitSettings,
      extraProperty, gIDevice, extraData);
    if bRet then
...
begin
  ...
  extraProperty := CreateSafeArray(2);
  deviceStatus := CreateSafeArray(2);
  bRet := gDeviceConnection.GetPropertyExt_2(DeviceProperty_MacAddress,
    extraProperty, gIDevice, deviceStatus);
  if bRet then
  begin
    for i := 0 to 5 do
...
begin
  // Get Welcome title
  extraProperty := CreateSafeArray(2);
  deviceStatus := gDeviceTools.GetBytesByNum(IntToStr(DeviceTile),
    NumberType_Int32Bit);
  bRet := gDeviceConnection.GetPropertyExt_2(DeviceProperty_WelcomeTitle,
    extraProperty, gIDevice, deviceStatus);
  if bRet then
  begin
...
begin
  // Get standby title
  extraProperty := CreateSafeArray(2);
  deviceStatus := gDeviceTools.GetBytesByNum(IntToStr(DeviceStandbyTitle),
    NumberType_Int32Bit);
  bRet := gDeviceConnection.GetPropertyExt_2(DeviceProperty_StandbyTitle,
    extraProperty, gIDevice, deviceStatus);
  if bRet then
  begin
...
begin
  // Set welcome title
  extraProperty := gDeviceTools.GetBytesByNum(IntToStr(DeviceTile),
    NumberType_Int32Bit);
  extraData := gDeviceTools.GetBytes(Trim(edtWelcome.Text));
  bRet := gDeviceConnection.SetPropertyExt_2(DeviceProperty_WelcomeTitle,
    extraProperty, gIDevice, extraData);
  if bRet then
  begin
...
begin
  // Set standby title
  extraProperty := gDeviceTools.GetBytesByNum(IntToStr(DeviceStandbyTitle),
    NumberType_Int32Bit);
  extraData := gDeviceTools.GetBytes(Trim(edtStandby.Text));
  bRet := gDeviceConnection.SetPropertyExt_2(DeviceProperty_StandbyTitle,
    extraProperty, gIDevice, extraData);
  if bRet then
  begin
...
begin
  ...
  extraProperty := CreateSafeArray(2);
  deviceStatus := gDeviceTools.GetBytesByNum(IntToStr(DeviceFirmwareVersion),
    NumberType_Int32Bit);
  bRet := gDeviceConnection.GetPropertyExt_2(DeviceProperty_FirmwareVersion,
    extraProperty, gIDevice, deviceStatus);
  if bRet then
  begin
...
begin
  // Get Model
  extraProperty := CreateSafeArray(2);
  deviceStatus := gDeviceTools.GetBytesByNum(IntToStr(DeviceModel),
    NumberType_Int32Bit);
  bRet := gDeviceConnection.GetPropertyExt_2(DeviceProperty_Model,
    extraProperty, gIDevice, deviceStatus);
  if bRet then
  begin
...
begin
  ...
  llength := StrToInt(gDeviceTools.GetStringByNum(chunkHeader, 4,
    NumberType_Int32Bit));
  ldownSize := llength + 4;
  ...
  begin
    ...
    soundType := gDeviceTools.GetBytesByNum
      (IntToStr(cbbSoundType.ItemIndex + 8), NumberType_Int32Bit);
    offSet := gDeviceTools.GetBytesByNum(IntToStr(i * lunit),
      NumberType_Int32Bit);
    extraProperty := CreateSafeArray(soundType.rgsabound[0].cElements +
      offSet.rgsabound[0].cElements - 1);
    ArrayCopy(soundType, 0, extraProperty, 0, soundType.rgsabound[0].cElements);
    ArrayCopy(offSet, 0, extraProperty, soundType.rgsabound[0].cElements,
      offSet.rgsabound[0].cElements);
    bresult := gDeviceConnection.SetPropertyExt_2(DeviceProperty_UploadSound,
      extraProperty, gIDevice, dataBytes);
    if not bresult then
    begin
      Result := False;
      Exit;
...
begin
  ...
  waveFormatSize := StrToInt(gDeviceTools.GetStringByNum(Buffer, 16,
    NumberType_Int32Bit));

  waveBytes := CreateSafeArray(17);
  ArrayCopy(Buffer, 20, waveBytes, 0, waveBytes.rgsabound[0].cElements);

  formatTag := StrToInt(gDeviceTools.GetStringByNum(waveBytes, 0,
    NumberType_UInt16Bit));
  if 1 <> formatTag then
  begin
  ...
  channels := StrToInt(gDeviceTools.GetStringByNum(waveBytes, 2,
    NumberType_UInt16Bit));
  if 1 <> channels then
  begin
  ...
  samplesPerSec := StrToInt(gDeviceTools.GetStringByNum(waveBytes, 4,
    NumberType_UInt32Bit));
  ...
  bitsPerSample := StrToInt(gDeviceTools.GetStringByNum(waveBytes, 14,
    NumberType_UInt16Bit));
